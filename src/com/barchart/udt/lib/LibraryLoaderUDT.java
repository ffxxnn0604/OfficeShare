...
public class LibraryLoaderUDT implements LibraryLoader {
	...
	public void load(final String targetFolder) throws Exception {
        ...
        System.loadLibrary("barchart-udt-core-armv7l-Linux-gpp");
		...
		try {
			log.info("Loading release libraries from: {}", targetFolder);
		...
		try {
			log.info("Loading staging libraries from: {}", targetFolder);
		...
		try {
			log.info("Loading testing libraries from: {}", targetFolder);
	...
	protected void loadAll(final List<String> sourceList, final String targetFolder) throws Exception {
